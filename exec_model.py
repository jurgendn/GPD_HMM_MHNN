...
def plot_training_prediction(
    ...
    plt.tight_layout()
    plt.savefig(
        f"images/visualized_{phase}.png", dpi=DPI, pad_inches=PAD_INCHES, bbox_inches="tight",
    )
...
def plot_test_states(model, idx: int, obs_series: np.ndarray, phase: str = "test") -> None:
    ...
    plt.tight_layout()
    plt.savefig(
        f"images/TSC_{idx}_{phase}.png", dpi=DPI, pad_inches=PAD_INCHES, bbox_inches="tight",
    )
...
def plot_test_comparisons(
    models: list, idx_list: Optional[list], obs_series: np.ndarray, phase: str = "test",
) -> None:
    ...
    plt.ylabel("Count Value", fontsize=FONTSIZE_LABEL, fontweight="bold")
    plt.title(
        "Test State Predictions vs New Observations", fontsize=FONTSIZE_TITLE, fontweight="bold",
    )
    plt.legend(fontsize=FONTSIZE_LEGEND, loc="best", frameon=True, shadow=True)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(
        f"images/TS_comparisons_{phase}.png", dpi=DPI, pad_inches=PAD_INCHES, bbox_inches="tight",
    )
...
def plot_multi_model_subplots(
    ...
    for plot_idx, idx in enumerate(idx_list):
        ...
        ax.set_title(
            f"Model {idx} ({plot_type.title()})", fontsize=FONTSIZE_TITLE, fontweight="bold",
        )
    ...
    plt.tight_layout()
    plt.savefig(
        f"images/TS_comparisons_subplots_{phase}.png"
        if plot_type == "predictions"
        else f"images/hidden_state_comparisons_subplots_{phase}.png",
        dpi=DPI,
...
def plot_test_comparisons_subplots(
    models: list, idx_list: Optional[list], obs_series: np.ndarray, phase: str = "test",
) -> None:
...
def plot_hidden_state_comparisons_subplots(
    models: list, idx_list: Optional[list], obs_series: np.ndarray, phase: str = "test",
) -> None:
...
def main() -> None:
    ...
    obs_train, obs_test = split_observations(
        obs_series, train_end=args.train_end, test_start=args.test_start,
    )
    ...
    plot_training_prediction(
        models[base_idx], obs_train, list(range(len(obs_train))), phase="train",
    )
